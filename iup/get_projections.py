...
def z_scale(nat_data, old_data):
    ...
    if isinstance(nat_data, pl.DataFrame):
        ...
        raise ValueError(
            "Raw data type is" + type(nat_data) + ", not pl.Series or numpy array."
        )
    ...
    if isinstance(old_data, pl.DataFrame):
        ...
        raise ValueError(
            "Data used to scale's type is"
            + type(old_data)
            + ", not pl.Series or numpy array."
        )
...
def inv_z_scale(z_data, old_data):
    ...
    if isinstance(z_data, pl.DataFrame):
        ...
        raise ValueError(
            "Scaled data type is" + type(z_data) + ", not pl.Series or numpy array."
        )
...
def get_projections(train_data, data_to_initiate, end_date, max_day_length=10):
    """
    ...
    scaled_x = (
        scaled_train.with_columns(interact=pl.col("previous") * pl.col("elapsed"))
        .select("previous", "elapsed", "interact")
        .to_numpy()
    )
    ...
    for date in date_series[1 : len(date_series) + 1]:
        raw_data = raw_data.with_columns(
            state=pl.lit("US").alias(
                "state"
            ),  # NOTE: pl.lit is an expression for literal value
        ...
        raw_data = raw_data.with_columns(daily=pl.lit(fitted_daily)).with_columns(
            cumulative=pl.col("cumulative") + pl.col("previous")
        )

        pred_data = pl.concat([pred_data, raw_data], how="vertical")
